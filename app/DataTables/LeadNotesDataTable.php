    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('title', function ($row) {
                if(!in_array('admin', user_roles()) && $row->ask_password == 1) {
                    return '<a href="javascript:;" class="ask-for-password" style="color:black;" data-lead-note-id="'.$row->id.'">' . ucwords($row->title) . '</a>';
                }
                else {
                    return '<a href="' . route('lead-notes.show', $row->id) . '" class="openRightModal" style="color:black;">' . ucwords($row->title) . '</a>';
                }
